...
const MARKETING_CARDS = {
  "1": { title: "ターゲットメディア選定", step: 1 },
  "2": { title: "MEO対策（Googleマップ）", step: 1 },
  "3": { title: "SNS運用方針", step: 1 },
  "4": { title: "オープニングキャンペーン", step: 1 },
  "5": { title: "グルメサイト活用", step: 2 },
  "6": { title: "リピーター施策（CRM）", step: 2 },
  "7": { title: "写真・クリエイティブ", step: 2 },
  "8": { title: "アナログ販促", step: 2 },
  "9": { title: "販促カレンダー", step: 3 },
  "10": { title: "販促予算", step: 3 },
  "11": { title: "情報発信ルーティン", step: 3 },
  "12": { title: "プレスリリース", step: 3 },
};

export function MarketingPage({ hideHeader = false, initialCardId }: { hideHeader?: boolean; initialCardId?: string | null }) {
  ...
  useEffect(() => {
    if (initialCardId && MARKETING_CARDS[initialCardId as keyof typeof MARKETING_CARDS]) {
  ...
  const loadStatuses = async () => {
    try {
      const { data, status } = await apiFetch<{ statuses: MarketingStatus[] }>("/api/marketing/status");
      if (status === 401) {
        clearAccessToken();
        router.replace("/");
        return;
  ...
  const getCardStatus = (cardId: string): MarketingStatus | undefined => {
  ...
  const content = (
    <>
      {!hideHeader && (
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-slate-900">販促軸の深掘り質問</h1>
          <p className="mt-2 text-sm text-slate-600">
            各カードをクリックして、AIと対話しながら販促の各項目を確定していきましょう。
          </p>
        </div>
      )}
        ...
        <section className="mb-12">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {cardsByStep.step1.map((cardId) => {
              ...
              const card = MARKETING_CARDS[cardId as keyof typeof MARKETING_CARDS];
...
function MarketingChatModal({ 
  ...
  const INITIAL_QUESTIONS: Record<string, string> = {
    ...
    "2": "【STEP 1/3 - 質問2】今や必須のGoogleビジネスプロフィール（MEO）の登録・運用計画を立てましょう。検索されたいキーワード（例: 「渋谷 カフェ」「梅田 ラーメン」）は何ですか？",
    "3": "【STEP 1/3 - 質問3】公式SNS（Instagram, X, TikTok, LINEなど）のうち、メインで運用するプラットフォームを1つ決め、どのような投稿をするか（写真メイン、店主のつぶやき等）を決めてください。",
    "4": "【STEP 1/3 - 質問4】開業直後の集客爆発を狙うためのオープン記念キャンペーン内容を決定してください。（例: ドリンク1杯無料、半額セール、ノベルティ配布）業態に合ったキャンペーン案を考えてみましょう。",
    "5": "【STEP 2/3 - 質問5】食べログ、ホットペッパー、ぐるなびなどのグルメ媒体（有料/無料）を利用しますか？利用する場合、どのサイトに登録しますか？",
    "6": "【STEP 2/3 - 質問6】一度来たお客様に再来店してもらうための仕組みを決めてください。（例: LINE公式アカウントへの登録誘導、ポイントカード、次回使えるクーポン）再来店率を上げる仕組みを考えてみましょう。",
    "7": "【STEP 2/3 - 質問7】販促に使用する料理や内装の写真は誰が撮りますか？（プロに依頼、自分で撮影、スマホで撮影）写真は「おいしさ」を伝える命です。",
    "8": "【STEP 2/3 - 質問8】近隣住民やオフィスワーカーに届けるためのアナログな手法（チラシ配布、看板、ショップカードの設置）をどう行いますか？",
    "9": "【STEP 3/3 - 質問9】オープンから3ヶ月間の販促スケジュール（プレオープン告知、オープン告知、1ヶ月記念等）の大枠を決定してください。標準的なスケジュールを参考に考えてみましょう。",
    "10": "【STEP 3/3 - 質問10】資金計画軸で確保した運転資金の中から、毎月の販促費（広告費、印刷代など）の上限をいくらに設定しますか？",
    "11": "【STEP 3/3 - 質問11】日々のSNS更新や口コミ返信を、「誰が」「いつ」「どのくらいの頻度」で行うか、現実的な運用ルールを決めてください。",
    "12": "【STEP 3/3 - 質問12】メディア（雑誌、Webニュース）に取り上げてもらうために、プレスリリースを配信しますか？配信する場合、一番の「ニュースになる売り」は何ですか？",
  };
  ...
  const handleGenerateSummary = async () => {
    ...
    setError(null);
    try {
      const { data, status } = await apiFetch<{ summary: string }>("/api/marketing/summary", {
        method: "POST",
        body: {
          card_id: cardId,
      ...
      <div className="relative w-full max-w-2xl mx-4 bg-white rounded-2xl shadow-2xl max-h-[90vh] flex flex-col ring-1 ring-black/5">
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-white rounded-t-2xl">
          <h2 className="text-lg font-bold text-slate-900">
            {MARKETING_CARDS[cardId as keyof typeof MARKETING_CARDS]?.title}
          </h2>
          <button
            onClick={onClose}
