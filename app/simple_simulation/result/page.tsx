  ...
  return (
    <div className="flex flex-1 flex-col gap-8 py-4">
      ...
      {immediateLoading ? (
        ...
        <>
          {/* コンセプト（句読点で区切って2行表示、手書き風フォント） */}
          {immediateData.concept_sub_comment && (
            <section>
              <h2 className="text-xl font-bold text-sky-700 mb-3">
                コンセプト
              </h2>
              <h3 className="text-2xl sm:text-3xl font-bold text-slate-900" style={{ fontFamily: 'var(--font-handwriting), sans-serif' }}>
                {(() => {
                  const text = immediateData.concept_sub_comment;
                  // 句読点（、。）で分割して2行にする
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-sky-50">
                    <th className="border border-slate-300 px-3 sm:px-4 py-2 sm:py-3 text-left text-sm font-semibold text-slate-700 whitespace-nowrap">
                      項目
                    </th>
                    <th className="border border-slate-300 px-3 sm:px-4 py-2 sm:py-3 text-right text-sm font-semibold text-slate-700 whitespace-nowrap">
                      金額・割合
                    </th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td className="border border-slate-300 px-3 sm:px-4 py-2 sm:py-3 text-sm text-slate-800 whitespace-nowrap">
                      想定月商
                    </td>
                    <td className="border border-slate-300 px-3 sm:px-4 py-2 sm:py-3 text-right text-sm font-semibold text-slate-900 whitespace-nowrap">
                      ¥{immediateData.financial_forecast?.monthly_sales?.toLocaleString() ?? '-'}
                    </td>
                  </tr>
                  <tr className="bg-slate-50">
                    <td className="border border-slate-300 px-3 sm:px-4 py-2 sm:py-3 text-sm text-slate-800">
                      <div className="whitespace-nowrap">家賃上限（目安）</div>
                      <div className="text-xs text-slate-500 mt-1">※この業態・利益率を前提とした目安です</div>
                    </td>
                    <td className="border border-slate-300 px-3 sm:px-4 py-2 sm:py-3 text-right text-sm font-semibold text-slate-900 whitespace-nowrap align-top">
