...
export default function ResultPage() {
  ...
  useEffect(() => {
    const submit = async () => {
      setLoading(true);
      setError(null);
      const { data, status } = await apiFetch<SimulationResult>(
  ...
  const conceptOneLiner =
    result?.concept_one_liner ||
    result?.concept_detail?.split("\n")[0] ||
    "仮コンセプトを生成中です";
  const conceptDetail =
    result?.concept_detail ||
    result?.concept_one_liner ||
    "簡易シミュレーションの回答をもとに仮コンセプトを生成します。";
  const fundsSummary =
    result?.funds_comment_text ||
    "客単価・席数・営業時間から、ざっくりと収支の注意点を提示します。";
  ...
  return (
    <div className="flex flex-1 flex-col gap-6">
      ...
      {loading ? (
        ...
        <div className="rounded-2xl bg-white p-6 shadow-sm text-rose-600">
          {error}
        </div>
      ) : result ? (
